    ...
    return (
        <form onSubmit={handleSearch} className="relative group w-full md:w-60">
            ...
            <input
                ...
                value={query}
                placeholder="Buscar"
                className="h-10 w-full md:w-60 bg-transparent border border-gray-400 pl-9 text-gray-100 rounded-md 
            text-body-sm outline-none transition-all duration-200 focus-within:border-blue-300 
            focus-within:ring-1 focus-within:ring-blue-300 placeholder:text-gray-300 
            placeholder:text-body-sm" />
            {query &&(
                <CircleX onClick={resetSearch} className="absolute w-4 h-4 top-1/2 -translate-y-1/2 right-3 text-gray-300 " />
            )}
